...
class kpts_decoder_multistructure(nn.Module):
    ...
    def create_graph(self, num_kpts: [int],
        ...
        #    raise ValueError('ERROR range_inner_outer must be an uneven number')
        if num_kpts[0]!=num_kpts[2]:
            raise ValueError('ERROR number of keypoints of epicardium must be equal to number of points for lv in v7')
